...
if [[ -n "${FT_COMM_BENCH_WORKLOADS:-}" ]]; then
  # shellcheck disable=SC2206
  WORKLOADS=($FT_COMM_BENCH_WORKLOADS)
else
...
write_run_info() {
...

  {
    ...
    echo
    env | sort | grep -E \
      '^(CUDA|FT_|HF_|NCCL|PYTHONPATH|RESULT_DIR|VLLM|MODEL|TP|PORT|HOST)=' \
      || true
  } >"$info_file"
...
write_backend_summary() {
  ...
  {
    echo "== $config =="
    grep "Using .*all-reduce backends.*tp:0" "$server_log" || true
...
trap - EXIT INT TERM
echo "Completed FT communicator benchmark run. Results: $RESULT_DIR" >&2
